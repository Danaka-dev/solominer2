...
{
    std::stringstream httpHost; httpHost << host() << ":" << port();

    headers.insert( { "Host" ,httpHost.str() });
    // headers.insert({ "Accept", "*/*" });

//--
    // headers.insert( { "Host" ,host() } );
    // headers.insert({ "Connection", "close" });
    headers.insert({ "User-Agent", "solominer/3.0" });
    ...
    ss << llhttp_method_name(static_cast<llhttp_method>(method)) << " /" << " HTTP/1.1" << kCRLF;
