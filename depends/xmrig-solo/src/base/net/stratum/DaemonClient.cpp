...
void xmrig::DaemonClient::onHttpData(const HttpData &data) {
    {
        LOG_INFO( "Got Http data from daemon \n" );
        LOG_INFO( "%s " CYAN_BOLD( "http%s://%s:%u " ) MAGENTA_BOLD( "\"%s %s\" " ) CSI "1;%dm%d" CLEAR BLACK_BOLD( " received: " ) CYAN_BOLD( "%zu" ) BLACK_BOLD( " bytes" ) ,
                tag() ,data.tlsVersion() ? "s" : "" ,data.host() ,data.port() ,llhttp_method_name( static_cast<llhttp_method>(data.method)) ,data.url.data() ,
