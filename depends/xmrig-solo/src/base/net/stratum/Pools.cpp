...
xmrig::IStrategy *xmrig::Pools::createStrategy(IStrategyListener *listener ,Controller *controller) const
{
    if (active() == 1) {
        for (const Pool &pool : m_data) {
            if (pool.isEnabled()) {
                return new SinglePoolStrategy(controller ,pool, retryPause(), retries(), listener);
            }
        }
    }

    auto strategy = new FailoverStrategy(controller ,retryPause(), retries(), listener);
    for (const Pool &pool : m_data) {
        if (pool.isEnabled()) {
            strategy->add(pool);
        }
