...
//-- bitcoin-blk
#include <bitcoin-blk/algo/common/byteswap.h>
...
bool xmrig::Job::setBlob( const uint8_t *blob ,size_t size ) {
    if( size > kMaxBlobSize ) return false;
...
bool xmrig::Job::setTarget(const char *target) {
    ...
    if (size == 4) {
        ...
        // m_target = *reinterpret_cast<const uint64_t*>( raw.data() +24 );
        m_target = bswap_64( *reinterpret_cast<const uint64_t *>(raw.data()) );
    } else {
        return false;
    }
