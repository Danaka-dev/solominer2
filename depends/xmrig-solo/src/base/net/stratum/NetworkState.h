...
class NetworkState : public INetworkState ,public StrategyProxy
{
public:
    ...
    inline uint64_t partial() const            { return m_partial; }
    inline uint64_t rejected() const            { return m_rejected; }
    // inline uint64_t failures() const            { return m_failures; }
...
public: ///-- INetworkState interface
    uint64_t acceptedShare() const override { return m_accepted; }
    uint64_t partialShare() const override { return m_partial; }
    uint64_t rejectedShare() const override { return m_rejected; }
    uint64_t failures() const override { return m_failures; }
    uint64_t hashes() const override { return m_hashes; }
    uint64_t diff() const override { return m_diff; }
...
private:
    ...
    uint64_t m_partial          = 0; //! partial block
};
