...
xmrig::Network::Network(Controller *controller ,IStrategyListener *strategyListener ) :
    m_controller(controller)
{
    m_contextId = newWorkContext();

    JobResults::setListener( m_contextId ,this ,controller->config()->cpu().isHwAES() );
    controller->addListener(this);

    StrategyProxy::setListener( strategyListener );
...
#   endif
    ...
    m_strategy = pools.createStrategy( m_state ,m_controller );
...
xmrig::INetworkState *xmrig::Network::state() {
    return m_state;
}
...
void xmrig::Network::onConfigChanged(Config *config, Config *previousConfig)
{
    ...
    delete m_strategy;
    m_strategy = config->pools().createStrategy( m_state ,m_controller );
    connect();
}


void xmrig::Network::onJob( IStrategy *strategy ,IClient *client ,const Job &job ,const rapidjson::Value &value ) {
    StrategyProxy::onJob( strategy ,client ,job ,value );
...
void xmrig::Network::onLogin(IStrategy *strategy ,IClient *client ,rapidjson::Document &doc ,rapidjson::Value &params ) {
    StrategyProxy::onLogin( strategy ,client ,doc ,params );
...
void xmrig::Network::onResultAccepted(IStrategy *strategy ,IClient *client ,const SubmitResult &result ,const char *error ) {
    ...
    } else if( strcmp( error ,"partial" )==0 ) {
        LOG_INFO("%s " BLUE_BOLD("partial") " (%" PRId64 "/%" PRId64 ") diff " WHITE_BOLD("%" PRIu64 "%s") " " BLACK_BOLD("(%" PRIu64 " ms)"),
                backend_tag(result.backend), m_state->accepted(), m_state->rejected(), diff, scale, result.elapsed);
    }
    else {
...
void xmrig::Network::onVerifyAlgorithm( IStrategy *strategy ,const IClient *client ,const Algorithm &algorithm ,bool *ok ) {
    StrategyProxy::onVerifyAlgorithm( strategy ,client ,algorithm ,ok );
...
#   endif
    ...
    { //! @note to avoid requiring Clients to properly set context, ugly tho...
        Job *nonConstJob = (Job*) &job;

        nonConstJob->contextId() = m_contextId;
    }
